...
namespace Engine
{
	...
	static Base::vec3 get_color(const hit& hit_item, const Base::texture& tex, const ray& rray)
	{
		...
		Base::cubemap texture_3d;
		to_cubemap(tex, texture_3d);
		if (hit_item.pmodel->s_type == Base::shape_type::SPHERE)
		{
			return hit_item.pmodel->surface_color * get_background_color(&texture_3d, rray.nhit);
		}
		if (hit_item.pmodel->s_type == Base::shape_type::BOX)
		{
			Base::box* pbox = (Base::box*)hit_item.shape;
			Base::vec3 dir = rray.phit - pbox->center;
	...
	static bool detect_hit(const world& models, ray& ray, hit& hit_item)
	{
		...
		{	
			bool has_hit = false;
			if (models.models[m].s_type == Base::shape_type::TRIANGLE)
			{
				has_hit = Triangle::detect_hit(models.models[m], ray, hit_item, tnear);
			}
			else if (models.models[m].s_type == Base::shape_type::SPHERE)
			{
				has_hit = Sphere::detect_hit(models.models[m], ray, hit_item, tnear);
			}
			else if (models.models[m].s_type == Base::shape_type::BOX)
			{
	...
	static double get_glow_by_shape(const model& model, const ray& shadow_ray, double& t0)
	{
		...
		else if (model.s_type == Base::shape_type::SPHERE)
		{
		...
		else if (model.s_type == Base::shape_type::BOX)
		{
	...
	static Base::vec3 get_background_reflection(const hit& hit_item, const ray& rray, const Base::cubemap* pcubemap)
	{
		...
		if (hit_item.pmodel->s_type == Base::shape_type::TRIANGLE)
		{
			...
			Base::vec3 displaced_centroid = centroid - rray.nhit;
