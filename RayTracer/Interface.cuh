...
namespace RayTracer
{
	...
	Core::vec3 get_color(const hit& hit_item, ray& rray, const texture& tex)
	{
		...
		rray.texcoord = get_texcoord(hit_item.shape, rray.phit, hit_item.pmodel->type);
	...
	bool detect_hit(const models& models, ray& ray, hit& hit_item)
	{
		...
		{	
			bool has_hit = false;
			if (models.models[m].type == Core::shape_type::TRIANGLE)
			{
				has_hit = Triangle::detect_hit(models.models[m], ray, hit_item, tnear);
			}
			else if (models.models[m].type == Core::shape_type::SPHERE)
			{
		...
		if (hit_item.pmodel->type == Core::shape_type::TRIANGLE)
		{
			triangle* ptriangle = (triangle*)hit_item.shape;
			ray.nhit = ptriangle->normal;
		}
		else if (hit_item.pmodel->type == Core::shape_type::SPHERE)
		{
	...
	{
		double glow = 1.0;
		if (model.type == Core::shape_type::TRIANGLE)
		{
		...
		else if (model.type == Core::shape_type::SPHERE)
		{
